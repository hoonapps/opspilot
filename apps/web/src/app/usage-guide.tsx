...
const usageSteps = [
  ...
  },
  {
    ...
    commands: ["문서 화면 > Markdown 등록", "등록하고 RAG 검증", "색인 품질 리포트 / 출처 적중 / 문서 일치율 확인", "문서 선택 > 영향 분석"]
  },
  {
    title: "청킹과 검색 품질 확인",
    body: "문서 화면에서 청크 커버리지와 헤딩 보존을 확인하고, 검색 화면에서 답변 생성 전에 후보 청크, 점수 격차, 출처 다양성, 컨텍스트 예산, 질문 변형 안정성을 확인합니다.",
    commands: [
      "문서 화면 > 품질 검사",
      "검색 화면 > 검색 미리보기",
      "권한 감사 허용/차단 후보 확인",
  ...
  {
    title: "평가와 배포 게이트 확인",
    body: "답변 단위 신뢰 게이트와 별도로, 평가, 문서 일치율, SLO, 배포 게이트, API 요청 성공률을 확인해 현재 지식 베이스가 배포 가능한지 판단합니다.",
    commands: [
      "pnpm eval",
      "pnpm retrieval-robustness:smoke",
...
const checklist = [
  ...
  },
  {
    ...
    body: "문서 화면의 색인 품질 리포트와 청크 미리보기, 검색 화면의 후보 청크 순위와 검색 강건성 리포트에서 실제 청크, 점수, 권한 차단 결과, 컨텍스트 예산 포함 여부, 질문 변형별 1순위 출처 안정성을 확인합니다."
  },
  {
