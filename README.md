...
공개 배포 URL 없이도 제품 동작을 같은 환경에서 재현할 수 있도록 한 번에 실행되는 증명 명령을 제공합니다.
...
| 확인할 것 | 어디서 보나 | 핵심 기준 |
| --- | --- | --- |
| 문서 수집 | `pnpm source-ingestion:smoke`, 웹 `문서` 화면 | URL/txt/PDF/Word가 저장, 청킹, 임베딩 후 1순위 출처가 됨 |
| RAG 답변 | 웹 `검색` 화면, `GET /answers/{id}/trace` | 답변에 출처, 문서 일치율, 근거 커버리지가 남음 |
...
- 큐/캐시: Redis, BullMQ
- 검색: pgvector 기본, Elasticsearch 선택형 하이브리드 검색, BM25/key-token 또는 embedding cosine 리랭킹
...
PDF/Word는 브라우저에서 base64 JSON으로 전송되므로 HTTP 요청 본문이 원본 파일보다 커집니다. 기본 요청 제한은 `OPSPILOT_REQUEST_BODY_LIMIT=20mb`이고, 추출 전 파일 자체는 10MB 이하만 허용합니다. 이 경계는 `pnpm source-ingestion-http:smoke`가 실제 `/documents/source` HTTP 요청으로 검증합니다.
