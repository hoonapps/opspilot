...
10. Agent가 출처 기반 답변을 생성합니다.
11. 답변과 출처 chunk 사이의 문서 일치율을 계산합니다.
12. 낮은 confidence, 출처 없음, 민감 작업은 `reviewReasons`로 구조화합니다.
13. 민감 작업은 `request_human_approval` 도구 호출과 approval record로 분리합니다.
14. 질문, 답변, 출처, 권한 감사, 도구 호출, approval, feedback을 저장합니다.
15. `GET /answers/:id/trace`는 저장된 답변 실행 내역을 복원합니다.
16. `GET /answers/:id/proof`는 trace를 pass/warn/fail 증거 패킷으로 요약합니다.
17. `GET /answers/:id/replay`는 현재 문서 기준으로 이전 답변의 drift를 확인합니다.
18. `GET /answers/:id/evidence-bundle`은 trace/proof/replay와 권한 재검사 결과를 묶고 SHA-256 해시를 붙입니다.
19. API request interceptor가 HTTP method, route, status, duration, actor hash를 저장합니다.
20. observability/배포 게이트 API가 운영 품질 상태와 API p95/오류율을 집계합니다.
