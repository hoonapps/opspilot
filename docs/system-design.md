...
22. `GET /answers/:id/evidence-bundle`은 추적/증명/재실행과 권한 재검사 결과를 묶고 SHA-256 해시를 붙입니다.
23. `GET /answers/:id/lineage`는 질문, 답변, 출처, 도구 호출, 승인, 피드백, 신뢰 게이트를 노드/엣지로 연결하고 SHA-256 해시를 붙입니다.
24. `GET /questions/:id/audit-bundle`은 답변 없는 장애 대응 작업 흐름까지 질문 기준으로 묶어 도구 호출 정책, 출처 계보, 권한 재검사, SHA-256 해시를 반환합니다.
25. API 요청 인터셉터가 HTTP 메서드, 경로, 라우트, 상태, 처리 시간, 호출자 해시를 저장합니다.
26. 관측성/배포 게이트 API가 운영 품질 상태와 API p95/오류율을 집계합니다.
