...
20. `GET /documents/revalidation-queue`는 문서 변경 이후 오래된 답변을 위험도와 우선순위 큐로 묶어 replay/lineage/quality gate 재검증 경로를 반환합니다.
21. `POST /documents/revalidation-runs`는 큐 항목 하나를 replay, 품질 게이트, 계보 그래프로 실제 재검증하고 종료/재검토/차단 판정과 리포트 해시를 저장합니다.
22. `GET /documents/revalidation-runs`는 저장된 재검증 실행 이력을 반환하고, 감사 원장은 이 실행을 `revalidation_run` 이벤트로 해시 체인에 포함합니다.
23. `GET /answers/:id/evidence-bundle`은 추적/증명/재실행과 권한 재검사 결과를 묶고 SHA-256 해시를 붙입니다.
24. `GET /answers/:id/lineage`는 질문, 답변, 출처, 도구 호출, 승인, 피드백, 신뢰 게이트를 노드/엣지로 연결하고 SHA-256 해시를 붙입니다.
25. `GET /questions/:id/audit-bundle`은 답변 없는 장애 대응 작업 흐름까지 질문 기준으로 묶어 도구 호출 정책, 출처 계보, 권한 재검사, SHA-256 해시를 반환합니다.
26. API 요청 인터셉터가 HTTP 메서드, 경로, 라우트, 상태, 처리 시간, 호출자 해시를 저장합니다.
27. 관측성/배포 게이트 API가 운영 품질 상태와 API p95/오류율을 집계합니다.
