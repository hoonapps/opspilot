...
`evidence-bundle`과 `audit-bundle`의 SHA-256 해시는 반환된 감사 payload 전체를 대상으로 합니다. 해시는 권한을 우회하는 서명이 아니라, 허용된 caller에게 보여준 trace/proof/replay/tool calling 증거가 나중에 바뀌지 않았는지 확인하기 위한 무결성 표시입니다.
