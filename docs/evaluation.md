...
- `documentAgreementScore`: 기본은 `token_overlap_v1` 토큰 겹침 기반 일치율이고, `DOCUMENT_AGREEMENT_METHOD=semantic_embedding`을 켜면 답변과 출처 청크를 같은 embedding provider로 벡터화해 가장 가까운 출처와의 `semantic_embedding_v1` 유사도로 계산합니다.
...
- `candidate`: OpenAI 또는 로컬 Transformers embedding으로 다시 계산한 같은 지표
- `delta`: candidate가 baseline 대비 얼마나 좋아지거나 나빠졌는지
- `rows[].localRankedSources`: 로컬 임베딩 기준 질문별 문서 순위
- `rows[].candidateRankedSources`: OpenAI 또는 Transformers 임베딩 기준 질문별 문서 순위
...
`pnpm embedding-hard:smoke`는 `seed/embedding-hard/documents`의 어려운 패러프레이즈 문서 세트를 임시로 색인합니다. 이 세트는 단어가 그대로 겹치지 않는 질문을 넣어 로컬 해시 임베딩의 한계를 드러내기 위한 테스트입니다. API key가 없을 때도 로컬 기준선 순위와 지표를 남기고, `OPENAI_API_KEY`가 있거나 `EMBEDDING_CANDIDATE_PROVIDER=transformers`를 설정하면 같은 문서와 질문으로 실제 임베딩 후보 지표까지 계산합니다.
