...
- `POST /documents/source`: URL, txt, Markdown, PDF, Word docx를 수집해 표준 Markdown으로 변환하고 기존 RAG 색인에 연결. 응답의 `quality`는 텍스트 추출 길이, 청크 생성, 청크 크기, 헤딩 신호, 검색 힌트, 보안 스캔을 `ready`/`attention`/`blocked`로 판정하고 추천 테스트 질문을 제공합니다.
