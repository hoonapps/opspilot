...
- `POST /documents/source`: URL, txt, Markdown, PDF, Word docx를 수집해 표준 Markdown으로 변환하고 기존 RAG 색인에 연결. 응답의 `quality`는 텍스트 추출 길이, 청크 생성, 청크 크기, 헤딩 신호, 검색 힌트, 보안 스캔을 `ready`/`attention`/`blocked`로 판정하고 추천 테스트 질문을 제공합니다. `provenance`는 원본 URL/파일명, 최종 URL, content type, 원본 바이트 크기, 추출 해시, 저장 content hash, 청크 수, URL 보안 가드 상태를 제공합니다. URL 수집은 기본적으로 localhost/private/link-local/multicast 주소와 내부망 redirect를 차단합니다.
