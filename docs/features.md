...
| 임베딩 비교 | 같은 질문/문서에서 로컬 해시 임베딩과 OpenAI/Transformers 임베딩의 검색 순위를 비교합니다. | `GET /evaluations/embedding-comparison`, `pnpm embedding-hard:smoke` |
| 리랭킹 | 검색 후보를 BM25/핵심 토큰 또는 임베딩 cosine similarity로 다시 정렬합니다. | `GET /evaluations/retrieval`, `pnpm rerank-challenge:smoke`, `pnpm semantic-rerank:smoke` |
...
PDF/Word 업로드는 base64 JSON 요청 크기와 실제 파일 크기 경계를 분리합니다. `OPSPILOT_REQUEST_BODY_LIMIT` 기본값은 `20mb`이고, 추출 대상 파일은 10MB 이하만 허용합니다. `pnpm source-ingestion-http:smoke`는 기본 Nest body limit 때문에 새 문서 등록이 413으로 막히지 않는지 HTTP 레벨에서 검증합니다.
...
9. 평가/품질 게이트/제품 리포트로 전체 상태를 검증합니다.

## 현재 프로젝트 성격

OpsPilot은 단순 챗봇이 아닙니다. 제품 확인에서 강조해야 할 포인트는 “LLM 답변을 만들었다”가 아니라 아래 세 가지입니다.
