...
3. 청크 미리보기와 색인 설명을 확인합니다.
4. 추천 테스트 질문으로 `/ask`를 실행합니다.
5. 답변에 출처와 문서 일치율이 표시되는지 확인합니다.
6. 권한이 낮은 사용자로 같은 질문을 던져 제한 문서가 빠지는지 확인합니다.
7. 민감 작업 질문을 던져 사람 승인 요청으로 분리되는지 확인합니다.
8. 평가/품질 게이트/포트폴리오 리포트로 전체 상태를 검증합니다.
